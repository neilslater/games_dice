...
class GamesDice::Probabilities
  ...
  # @return [GamesDice::Probabilities] new distribution
  def repeat_n_sum_k_old n, k, kmode = :keep_best
    n = Integer( n )
    k = Integer( k )
    raise "Cannot combine probabilities less than once" if n < 1
    if k >= n
    ...
    new_offset = @offset * k
    d = n - k
    (0...@probs.count).to_a.repeated_combination( k ).each do |keepers|
      min_keeper = keepers.min
      p_not_kept = ( kmode == :keep_best ? p_lt( min_keeper + @offset ) : p_gt( min_keeper + @offset ) )
      r_combo = keepers.inject(:+)
      [min_keeper,:not_kept].repeated_combination( d ).each do |discards|
        sequence = keepers + discards
        new_probs[r_combo] += p_sequence( sequence, { :not_kept => p_not_kept } ) * GamesDice::Combinations.count_variations( sequence )
      end
    end
  ...
  # Calculates probability of getting exact sequence of results for this distribution,
  # offsets already factored out . . . specials are pre-calculated symbols (covering combinations
  # of equivalent results for example)
  def p_sequence array, specials = { :not_kept => 0.0 }
    array.inject(1.0) do |ps,i|
      if specials[i]
        ps * specials[i]
      else
        ps * @probs[i]
