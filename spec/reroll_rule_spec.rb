...
describe GamesDice::RerollRule do
  describe "#new" do
    ...
    it "should reject inconsistent operator/value pairs for a trigger" do
      expect(lambda { GamesDice::RerollRule.new( 5, :member?, :reroll_subtract ) }).to raise_error( ArgumentError )
      expect(lambda { GamesDice::RerollRule.new( (1..5), :>, :reroll_replace ) }).to raise_error( ArgumentError )
    end

    it "should reject bad re-roll types" do
      expect(lambda { GamesDice::RerollRule.new( 5, :>, :reroll_again ) }).to raise_error( ArgumentError )
      expect(lambda { GamesDice::RerollRule.new( (1..5), :member?, 42 ) }).to raise_error( ArgumentError )
    end
  end
