...
describe GamesDice::DieResult do
  ...
  describe ".new" do
    ...
    it "should not accept a param that cannot be coerced to Integer" do
      expect(lambda { GamesDice::DieResult.new([]) }).to raise_error( TypeError )
      expect(lambda { GamesDice::DieResult.new('N') }).to raise_error( ArgumentError )
    end

    it "should not accept unknown reasons for making a roll" do
      expect(lambda { GamesDice::DieResult.new(8,'wooo') }).to raise_error( ArgumentError )
      expect(lambda { GamesDice::DieResult.new(8,:frabulous) }).to raise_error( ArgumentError )
    end
  end
  ...
  describe "#add_roll" do
    context "starting from 'no results yet'" do
      ...
      it "should not accept a first param that cannot be coerced to Integer" do
        expect(lambda { die_result.add_roll([]) }).to raise_error( TypeError )
        expect(lambda { die_result.add_roll('N') }).to raise_error( ArgumentError )
      end

      it "should not accept an unsupported second param" do
        expect(lambda { die_result.add_roll(5,[]) }).to raise_error( ArgumentError )
        expect(lambda { die_result.add_roll(15,:bam) }).to raise_error( ArgumentError )
      end
    end
    ...
    context "starting with an initial result" do
      ...
      context "add another basic roll" do
        it "should replace an initial result, as if the die were re-rolled" do
          ...
          expect(die_result.roll_reasons).to eql [:basic, :basic]
        end
      end
      ...
      context "exploding dice" do
        it "should add to value when exploding up" do
          ...
          expect(die_result.roll_reasons).to eql [:basic, :reroll_add]
        end
        ...
        it "should subtract from value when exploding down" do
          ...
          expect(die_result.roll_reasons).to eql [:basic, :reroll_subtract]
        end
      end
      ...
      context "re-roll dice" do
        it "should optionally replace roll unconditionally" do
          ...
          expect(die_result.roll_reasons).to eql [:basic, :reroll_replace]

          die_result.add_roll( 5, :reroll_replace )
          expect(die_result.value).to eql 5
          expect(die_result.rolls).to eql [7,2,5]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_replace, :reroll_replace]
        end
        ...
        it "should optionally use best roll" do
          ...
          expect(die_result.rolls).to eql [7,2]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_use_best]

          die_result.add_roll( 9, :reroll_use_best )
          expect(die_result.value).to eql 9
          expect(die_result.rolls).to eql [7,2,9]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_use_best, :reroll_use_best]
        end
        ...
        it "should optionally use worst roll" do
          ...
          expect(die_result.value).to eql 4
          expect(die_result.rolls).to eql [7,4]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_use_worst]

          die_result.add_roll( 5, :reroll_use_worst )
          expect(die_result.value).to eql 4
          expect(die_result.rolls).to eql [7,4,5]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_use_worst, :reroll_use_worst]
        end
      end
      ...
      context "combinations of reroll reasons" do
        it "should correctly handle valid reasons for extra rolls in combination" do
          ...
          expect(die_result.roll_reasons).to eql [:basic, :reroll_add, :reroll_subtract]

          die_result.add_roll( 12, :reroll_replace )
          expect(die_result.value).to eql 12
          expect(die_result.rolls).to eql [7,10,3,12]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_add, :reroll_subtract, :reroll_replace]

          die_result.add_roll( 9, :reroll_use_best )
          expect(die_result.value).to eql 12
          expect(die_result.rolls).to eql [7,10,3,12,9]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_add, :reroll_subtract, :reroll_replace, :reroll_use_best]

          die_result.add_roll( 15, :reroll_add)
          expect(die_result.value).to eql 27
          expect(die_result.rolls).to eql [7,10,3,12,9,15]
          expect(die_result.roll_reasons).to eql [:basic, :reroll_add, :reroll_subtract, :reroll_replace, :reroll_use_best, :reroll_add]
        end
      end
    end
  end
  ...
  it "should support comparison with >,<,>=,<= as if it were an integer, based on #value" do
    ...
    expect((14 > die_result)).to eql true
    expect((die_result >= 7)).to eql true
    expect((9.5 >= die_result)).to eql true
    expect((die_result < 3)).to eql false
    expect((14 < die_result)).to eql false
