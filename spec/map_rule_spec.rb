...
describe GamesDice::MapRule do
  describe "#new" do
    ...
    it "should reject inconsistent operator/value pairs for a trigger" do
      expect(lambda { GamesDice::MapRule.new( 5, :member?, -1 ) }).to raise_error( ArgumentError )
      expect(lambda { GamesDice::MapRule.new( (1..5), :>, 12 ) }).to raise_error( ArgumentError )
    end

    it "should reject non-Integer map results" do
      expect(lambda { GamesDice::MapRule.new( 5, :>, :reroll_again ) }).to raise_error( TypeError )
      expect(lambda { GamesDice::MapRule.new( (1..5), :member?, 'foo' ) }).to raise_error( TypeError )
    end
  end
